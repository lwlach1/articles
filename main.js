...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdweGR2ZHVlZ3NzdXlzb2phd2N1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDk0NjU1OTksImV4cCI6MjA2NTA0MTU5OX0.UpWdqXrhF3WfARNIfCt2GWNkD8qWKA_ZixSkoSp4qfY';
...
function renderArticles() {
  ...
  articles.forEach((article) => {
    ...
    const updatedDate = article.updated_at && article.updated_at !== article.created_at ? new Date(article.updated_at).toLocaleString('pl-PL') : null;
    articleElement.innerHTML = `
      <h2 class="text-xl font-bold mb-2">${article.title}</h2>
      ${article.subtitle ? `<h3 class="text-md text-gray-600 mb-2">${article.subtitle}</h3>` : ''}
      <div class="text-sm text-gray-500 mb-2">Autor: ${article.author || 'brak'} | Utworzono: ${createdDate}${updatedDate ? ' | Zaktualizowano: ' + updatedDate : ''}</div>
